...
if (!empty($gSetting['ims_wp_woocom_button']) == 1 && function_exists('is_product')) :
    if (is_product()) :
...
$id =  $argu['id'];
$m_Width = get_post_meta($id, '_ims_wtp_member_acc_', true);
$preTimeZ = get_option('gmt_offset');
$hours = ceil($preTimeZ);
$member_text = get_post_meta($id, '_ims_wtp_member_acc_', true);
...
$allDay = $m_Width['ims_member_all_days'];
if ($allDay) {
    $mClass = ($allDay) ? 'awc_online' : 'awc_offline';
} else {

    $wDays = $m_Width['ims_member_weekdays'];
...
if ($mClass == 'awc_online') {
    ...
    $mUrl .= '.whatsapp.com/send?phone=' . $m_Width['ims_member_number'] . '&text=' . $text;
    $target = 'target="_blank"';
    $stImg = plugin_dir_url(__DIR__) . 'images/mOnline.png';
    $cOnline = "Online";
    $otext = $m_Width['ims_member_btntext'];
} else {
    $mUrl = 'javascript:;';
    $target = '';
    $stImg = plugin_dir_url(__DIR__) . 'images/mOnline.png';
...
($gSetting['ims_whatsapp_btn_color']) ? $bgcolor = $gSetting['ims_whatsapp_btn_color'] : '';
($gSetting['ims_whatsapp_txtbtn_color']) ? $color = $gSetting['ims_whatsapp_txtbtn_color'] : '';
...
if ($publish_post->post_status == 'publish') { ?>
    <div class="acw_suser <?php echo $mClass; ?> ">
        <a <?php echo $target; ?> href="<?php echo $mUrl ?>" class="ims_what_widget_outer">
            <div class="ims_widget_user_image">
                <div class="ims_image_outer">
                    <img src="<?php echo $image ?>">
                </div>
                <img src="<?php echo $stImg ?>" class="whats_app_icon">

            </div>
            <div class="ims_widget_content_">
                <h4><?php echo get_the_title($id) ?> <span><?php echo $cOnline; ?></span></h4>

                <?php if ($mClass == 'awc_online') : ?>
                    <p> <?php _e($m_Width['ims_member_btntext']);
                        echo '</p>';
                    else : echo "<p> $offMsg";
                    endif; ?></p>
            </div>
